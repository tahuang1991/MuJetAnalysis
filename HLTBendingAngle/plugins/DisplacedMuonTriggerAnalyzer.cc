...
#include "MuJetAnalysis/HLTBendingAngle/src/Ptassignment.cc"
...
DisplacedMuonTriggerAnalyzer::analyzeTrackEfficiency(SimTrackMatchManager& match, int trk_no, int st)
{
 ...
 //CSC SimHits Start here
  ...
  for(auto d: csc_simhits)
  {
	...
	edm::PSimHitContainer simhits_v2 = match_sh.hitsInChamber(rawId);
	simhits.insert(simhits.end(), simhits_v2.begin(), simhits_v2.end());
    ...
    if (id.chamber()%2==1){
	 oddeven[id.station()-1]=1; 
     	 gp_sh_odd[id.station()-1] = match_sh.simHitsMeanPosition(simhits);
    	 gv_sh_odd[id.station()-1] = match_sh.simHitsMeanMomentum(simhits);
      }else {
	 oddeven[id.station()-1]=2; 
     	 gp_sh_even[id.station()-1] = match_sh.simHitsMeanPosition(simhits);
    	 gv_sh_even[id.station()-1] = match_sh.simHitsMeanMomentum(simhits);
      }
  ...
  if (has_csc_sh[0] and has_csc_sh[1]){
     ...
     if (oddeven[0]==1 and oddeven[1]==2){
        gp1=gp_sh_odd[0];
        gp2=gp_sh_even[1];
        gv1=gv_sh_odd[0];
        gv2=gv_sh_even[1];
	npar=0;
	if (has_csc_sh[2] and oddeven[2]==2) 
        	gp3=gp_sh_even[2];
     }else if (oddeven[0]==1 and oddeven[1]==1) {
        gp1=gp_sh_odd[0];
        gp2=gp_sh_odd[1];
        gv1=gv_sh_odd[0];
        gv2=gv_sh_odd[1];
	npar=1;
	if (has_csc_sh[2] and oddeven[2]==1) 
        	gp3=gp_sh_odd[2];
    }else if (oddeven[0]==2 and oddeven[1]==2){
        gp1=gp_sh_even[0];
        gp2=gp_sh_even[1];
        gv1=gv_sh_even[0];
        gv2=gv_sh_even[1];
	...
	if (has_csc_sh[2] and oddeven[2]==2) 
        	gp3=gp_sh_even[2];
    }else if (oddeven[0]==2 and oddeven[1]==1){
        gp1=gp_sh_even[0];
        gp2=gp_sh_odd[1];
        gv1=gv_sh_even[0];
        gv2=gv_sh_odd[1];
	...
	if (has_csc_sh[2] and oddeven[2]==1) 
     ...
     pt_direction_tmp=Ptassign_Direction(csc_bending_angle_12, gp2.eta(), npar);  
      std::cout <<" simeta "<<t.momentum().eta() <<" eta1 "<< gp1.eta() <<" phi1 "<< gp1.phi() <<" gv_phi1 "<< gv1.phi() <<" eta2 "<< gp2.eta() <<" phi2 "<< gp2.phi() <<" gv2_phi "<< gv2.phi()<< std::endl;
     if (has_csc_sh[2] and oddeven[1]==oddeven[2]){
	std::cout <<"station3 "<<(oddeven[2]==1? " odd ":" even ")<<" eta3 "<< gp3.eta() <<" phi3 "<< gp3.phi() << std::endl;
	pt_position_tmp=Ptassign_Position_gp(gp1, gp2, gp3, gp2.eta(), npar); //t.momentum().eta() 
	}

     std::cout <<"case "<< st <<" eta "<< gp2.eta() <<" has csc sh st12 npar "<< npar <<" simpt "<< t.momentum().pt() <<" pt_position "<< pt_position_tmp << " pt_direction "<< pt_direction_tmp <<std::endl;
     if (pt_direction_tmp<8 and t.momentum().pt()>10 and fabs(gp2.eta())<2.4 and fabs(gp2.eta())>1.6) 
 ...
 for(auto d: csc_simhits)
 {
    ...
    if (id.station() == 1){
    	...
    	GlobalPoint hitGp = match_sh.simHitsMeanPosition(simhits);
  ...
  if (etrk_csc_[st].csc_st1_nlayerscsc>3 and etrk_csc_[st].csc_st3_nlayerscsc>3){
	etrk_csc_[st].csc_deltaphi_gp_13 = deltaPhi(etrk_csc_[st].csc_st1_gp_phi ,etrk_csc_[st].csc_st3_gp_phi);
	etrk_csc_[st].csc_bending_angle_13 = deltaPhi(etrk_csc_[st].csc_st1_gv_phi ,etrk_csc_[st].csc_st3_gv_phi);
	}

  if (etrk_csc_[st].csc_st1_nlayerscsc>3 and etrk_csc_[st].csc_st2_nlayerscsc>3 and etrk_csc_[st].csc_st3_nlayerscsc>3){
	float anglea = etrk_csc_[st].csc_st2_gp_phi;
	float newyst1 = -etrk_csc_[st].csc_st1_gp_x*sin(anglea) + etrk_csc_[st].csc_st1_gp_y*cos(anglea);
	float newxst1 = etrk_csc_[st].csc_st1_gp_x*cos(anglea) + etrk_csc_[st].csc_st1_gp_y*sin(anglea);
	float newyst2 = -etrk_csc_[st].csc_st2_gp_x*sin(anglea) + etrk_csc_[st].csc_st2_gp_y*cos(anglea);
	float newxst2 = etrk_csc_[st].csc_st2_gp_x*cos(anglea) + etrk_csc_[st].csc_st2_gp_y*sin(anglea);
	float newyst3 = -etrk_csc_[st].csc_st3_gp_x*sin(anglea) + etrk_csc_[st].csc_st3_gp_y*cos(anglea);
	float newxst3 = etrk_csc_[st].csc_st3_gp_x*cos(anglea) + etrk_csc_[st].csc_st3_gp_y*sin(anglea);
