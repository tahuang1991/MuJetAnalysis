...
#_____________________________________________________________________________________________________________________________________________________________________________________________________________________________
def numerator_Direction(dxy_min = 0, dxy_max = 5, eta_min = 1.6, eta_max = 1.7, minpt=0, maxpt = 500, stationx=0, parity_case=0, minptcut=0):

    return AND(denominator(dxy_min, dxy_max, eta_min, eta_max, minpt, maxpt, stationx, parity_case), pT_from_Direction(eta_min, stationx, parity_case, minptcut))




#_____________________________________________________________________________________________________________________________________________________________________________________________________________________________
def DeltaPhi_Scatter(file, dir, xaxis, yaxis, x_bins, y_bins, etamin, etamax, minpt, maxpt, ctau, stationx, parity_case):
    ...
    b1.GetYaxis().SetTitle("|\Delta \phi _{ST1 - ST2 }| ^{-1}")
    b1.SetTitle("|\Delta \phi _{ST1 - ST2}| ^{-1} vs p^{SimT}_{T}, ct %d mm"%ctau+" on %.1f < | \eta | < %.1f"%(etamin,etamax))
    b1.SetStats(1)    
    ...
    r1.Fit("pol1", "F LL", "same", 3.01, test_fit)
    r1.GetXaxis().SetTitle("p_{T} of Simulated Muon Track [GeV]")
    r1.GetYaxis().SetTitle("|\Delta \phi _{ST1 - ST2 }| ^{-1}")
    r1.SetTitle("|\Delta \phi _{ST1 - ST2}| ^{-1} vs p^{SimT}_{T}, ct %d mm"%ctau+" on %.1f < | \eta | < %.1f"%(etamin,etamax))
...
#_____________________________________________________________________________________________________________________________________________________________________________________________________________________________
def DeltaPhi_Efficiency(file, dir, etamin, etamax, ctau, stationx, parity_case,  minptcut):
    ...
    t1.Draw("pt_SimTrack>> SimTrack_pt_numerator_0dxy5",numerator_Direction(0, 5, etamin, etamax, 0, 500, stationx, parity_case, minptcut))
    ...
    t1.Draw("pt_SimTrack>> SimTrack_pt_numerator_10dxy30",numerator_Direction(5, 30, etamin, etamax, 0, 500, stationx, parity_case, minptcut))
    ...
    t1.Draw("pt_SimTrack>> SimTrack_pt_numerator_50dxy500",numerator_Direction(30, 500, etamin, etamax, 0, 500, stationx, parity_case, minptcut))
    ...
    b1.GetXaxis().SetTitle(" p_{T} of Simulated Muon Track [GeV]")
    b1.SetTitle(" p_{T} Reco. Efficiency, ct %d mm"%(ctau)+", %.1f < | \eta_{gp} | < %.1f using \Delta \phi "%(etamin,etamax))
    b1.SetStats(0)
...
for x in range (0, 4):
        ...
        #DeltaPhi_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.6, 1.8, 0, 500 , ctau, stx, x)
        #DeltaPhi_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 1.8, 2.0, 0, 500 , ctau, stx, x)
        #DeltaPhi_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 2.0, 2.2, 0, 500 , ctau, stx, x)
        #DeltaPhi_Scatter("../ct%d.root"%ctau,tree,xaxis,yaxis,x_bins,y_bins, 2.2, 2.4, 0, 500 , ctau, stx, x)

        DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.6, 1.8, ctau, stx, x, 10)
        DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.8, 2.0, ctau, stx, x, 10)
        DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.0, 2.2, ctau, stx, x, 10)
        DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.2, 2.4, ctau, stx, x, 10)

'''

DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.6, 1.8, 0, minptcut, ctau, 0)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.6, 1.8, 0, minptcut, ctau, 1)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.8, 2.0, 0, minptcut, ctau, 0)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 1.8, 2.0, 0, minptcut, ctau, 1)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.0, 2.2, 0, minptcut, ctau, 0)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.0, 2.2, 0, minptcut, ctau, 1)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.2, 2.4, 0, minptcut, ctau, 0)
DeltaPhi_Efficiency("../ct%d.root"%ctau,tree, 2.2, 2.4, 0, minptcut, ctau, 1)
'''
